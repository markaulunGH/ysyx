...
class DS extends Module
{
    ...
    val src2_is_imm = inst_auipc || inst_addi

    io.reg_r.raddr1 := inst(19, 15)
    io.reg_r.raddr2 := inst(24, 20)

    for (i <- 0 until 19)
    {
        io.ds_es.alu.alu_op(i) := 0.U
    }
    io.ds_es.alu.alu_op(0) := inst_auipc || inst_addi
    io.ds_es.alu.alu_src1 := Mux(src1_is_pc, io.fs_ds.pc, io.reg_r.rdata1)
    io.ds_es.alu.alu_src2 := Mux(src2_is_imm, inst(31, 20), io.reg_r.rdata2)
