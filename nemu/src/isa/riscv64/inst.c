...
enum {
  TYPE_R, TYPE_I, TYPE_S, TYPE_B, TYPE_U, TYPE_J, TYPE_C,
  TYPE_N, // none
};
...
static void decode_operand(Decode *s, int *dest, word_t *src1, word_t *src2, word_t *imm, int type) {
  ...
  *dest = rd;
  switch (type) {
    ...
    case TYPE_C: *src1 = rs1;      immI(); break;
  }
}
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (imm + src1) & ~1, R(dest) = s->snpc);
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = BITS(Mr(src1 + imm, 1), 7, 0));
  ...
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(dest) = (__int128_t) src1 * (__int128_t) src2);
  ...
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , I, R(dest) = csr(imm), csr(imm) &= ~src1);
  INSTPAT("??????? ????? ????? 101 ????? 11100 11", csrrwi , C, R(dest) = csr(imm), csr(imm) = src1);
  INSTPAT("??????? ????? ????? 110 ????? 11100 11", csrrsi , C, R(dest) = csr(imm), csr(imm) |= src1);
  INSTPAT("??????? ????? ????? 111 ????? 11100 11", csrrci , C, R(dest) = csr(imm), csr(imm) &= ~src1);
